(ns mining-engineers.actor
  "MiningEngineerActor — the StateGraph wiring for ISCO-08 2146 mining engineers,
  metallurgists and related professionals support. Implements the core itonami
  actor pattern:
  ...
  (:require [mining-engineers.advisor :as advisor]
...
(defn build-graph
  "Constructs the StateGraph for mining engineer support.
...
(defn run-request!
  ...
  (let [state {:request request :context context :store store-instance}]
    ...
    (-> state
        ...
        ((fn [s]
           (let [proposal (advisor/-advise (:advisor graph-config) store-instance request)]
             (assoc s :proposal proposal))))
        ((fn [s]
           (let [verdict (governor/check request context (:proposal s) store-instance)]
...
(defn approve!
  "Marks an escalated request as approved and resumes graph execution.
   In a real checkpointable implementation, this would resume from the
   :request-approval interrupt. For this stub, it returns a committed outcome."
  [state]
  (assoc state :outcome :committed))
